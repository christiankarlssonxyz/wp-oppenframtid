(function () {
    ...
    results.innerHTML = '<p class="search-results__status">Laddar inlägg…</p>';

    fetch(blogtreeSearch.restUrl + 'wp/v2/posts?per_page=100&_fields=id,title,date,link,excerpt')
        .then(function (r) { return r.json(); })
        .then(function (data) {
            allPosts   = data || [];
            postsReady = true;

            // Prefyll från URL-parameter ?q=
    ...
    function score(pattern, str) {
        ...
        for (var si = 0; si < s.length && pi < p.length; si++) {
            if (p[pi] === s[si]) { pts += 10 + consec * 6; consec++; pi++; }
            else { consec = 0; }
        }
    ...
    function runSearch() {
        ...
        var limit    = dateLimit(dateSel.value);
        ...
        var pool = allPosts.filter(function (post) {
            return !limit || new Date(post.date) >= limit;
        });

        if (query) {
            var scored = pool.map(function (post) {
    ...
    function render(posts, query) {
        ...
        posts.forEach(function (post) {
            var title   = stripTags(post.title.rendered);
            var excerpt = stripTags(post.excerpt.rendered).replace(/\s+/g, ' ').trim().substring(0, 160);
            ...
            if (excerpt) html += '<span class="search-results__excerpt">' + highlight(excerpt, query) + '</span>';
            html += '</a></li>';
        });
        html += '</ul>';
